...
public class DrawingPanel extends JPanel {
	...
	public DrawingPanel (AntSimulator par) {
		parent = par;
	}
	...
	public void paintComponent (Graphics g) {
		...
		if (parent.toRepaint == null) {
			...
			for (int x = 0; x < parent.LANDSCAPE_DIMENSION; x++) {
				for (int y = 0; y < parent.LANDSCAPE_DIMENSION; y++) {
					g.drawRect(x*parent.GRID_SIZE, (y*parent.GRID_SIZE), parent.GRID_SIZE, parent.GRID_SIZE);
					if (parent.landscape.get(x, y) == 1) {
						g.fillRect(x*parent.GRID_SIZE, (y*parent.GRID_SIZE), parent.GRID_SIZE, parent.GRID_SIZE);
		...
		System.out.println ("Drawing square " + parent.toRepaint);
		...
		g.fillRect(parent.toRepaint.x*parent.GRID_SIZE, (parent.toRepaint.y*parent.GRID_SIZE), parent.GRID_SIZE, parent.GRID_SIZE);
		g.setColor(Color.black);
		g.drawRect(parent.toRepaint.x*parent.GRID_SIZE, (parent.toRepaint.y*parent.GRID_SIZE), parent.GRID_SIZE, parent.GRID_SIZE);
	...
	public void paintSquare (int x, int y, int a) {
		System.out.println ("Painting " + x + " " + y);
		//parent.toRepaint = new Coord (x, y);
		//parent.antDirection = a;
		//paint (getGraphics());
		//repaint (x*parent.GRID_SIZE, y*parent.GRID_SIZE, parent.GRID_SIZE, parent.GRID_SIZE);
		repaint();
		System.out.println ("Painted");
	}
	
	public void paintAnt (Graphics g, int x, int y, int r) {
